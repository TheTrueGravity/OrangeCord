    ...
    exec(client, message, args, args1) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (utils_1.OrangeUtils.Servers[message.guildId].modRoles.includes(id)) {
                message.reply(`The role ${role.name} (${role.id}) is already a mod role!!`);
                return;
            }
            utils_1.OrangeUtils.Servers[message.guildId].modRoles.push(id);
            message.reply(`Successfully added the mod role ${role.name} (${role.id})!`);
        });
    }
};
